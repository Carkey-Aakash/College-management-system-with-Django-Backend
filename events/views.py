...
#                         message=f"The event '{event.title}' has been completed successfully.",
...
#                 # Match (class_name AND year) OR (class_name AND semester)
#                 query = query & (
...
#             # Notify only the targeted students
#             for student in students:
#                 create_notification(
#                     recipient=student,
#                     title=f"New Class Event: {event.title}",
#                     message=f"A new class-level event '{event.title}' has been scheduled for your class.",
...
#                         student.email,
#                         f"New Class Event: {event.title}",
#                         f"A new class-level event '{event.title}' has been scheduled for your class. Check it out!"
#                     )
...
#                     message=f"A new event '{event.title}' has been submitted for approval.",
#                     notification_type='event_created',
#                     event=event
#                 )
...
#                     send_email_notification(
#                         campus_chief.email,
#                         f"New Event Submitted: {event.title}",
#                         f"A new event '{event.title}' has been submitted for your approval."
#                     )
...
#     serializer_class = EventSerializer
#     permission_classes = [permissions.IsAuthenticated, IsEventManagerOrReadOnly]
...
#                 create_notification(
#                     recipient=student,
#                     title=f"Update in Event: {event.title}",
...
#                         f"The event '{event.title}' has been updated. Please check the details."
#                     )
...
#                     message=f"The event '{event.title}' has been updated and requires your review.",
#                     notification_type='event_update',
#                     event=event
#                 )
#                 if campus_chief.email:
#                     send_email_notification(
#                         campus_chief.email,
#                         f"Event Update Request: {event.title}",
#                         f"The event '{event.title}' has been updated and may need your attention."
#                     )
...
#     # """
#     # Approve, reject, or cancel an event.
#     # - Class level events cannot be approved/rejected/cancelled (auto-approved earlier).
#     # - Already approved events cannot be approved again.
#     # - Only campus-chief can approve/reject/cancel faculty/college events.
#     # """
...
#             title=f"Your Event '{event.title}' was {event.status}",
#             message=f"Campus Chief has {event.status} your event '{event.title}'.",
...
#                 f"Event '{event.title}' {event.status.capitalize()}",
#                 f"Your event '{event.title}' has been {event.status} by the Campus Chief."
#             )
...
#                 create_notification(
#                     recipient=student,
#                     title=f"New Approved Event: {event.title}",
#                     message=f"The {event.event_level} level event '{event.title}' has been approved and is now open.",
...
#                     send_email_notification(
#                         student.email,
#                         f"New Approved Event: {event.title}",
#                         f"The {event.event_level} level event '{event.title}' has been approved and is now open for registration."
#                     )
...
#     try:
#         # Get college student record by username
...
#         title=f"Registration Successful: {event.title}",
...
#         send_email_notification(
#             request.user.email,
#             f"Registration Successful: {event.title}",
...
# #     if not qr_data:
# #         return Response({'error': 'QR code data required'}, 
# #                        status=status.HTTP_400_BAD_REQUEST)
    
# #     # Verify QR code belongs to this event
# #     if not qr_data.startswith(f'eventify_attendance_{event.id}'):
# #         return Response({'error': 'Invalid QR code for this event'}, 
# #                        status=status.HTTP_400_BAD_REQUEST)
...
# #     except EventRegistration.DoesNotExist:
# #         return Response({'error': 'Registration not found or not confirmed'}, 
# #                        status=status.HTTP_404_NOT_FOUND)
...
#     # Get statistics for an event (Organizer/Admin only)
